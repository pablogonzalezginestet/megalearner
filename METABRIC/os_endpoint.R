...
metabric = data.clin[c('OS_MONTHS','OS_STATUS','AGE_AT_DIAGNOSIS','TUMOR_SIZE',"GRADE","LYMPH_NODES_EXAMINED_POSITIVE", "INFERRED_MENOPAUSAL_STATE","HORMONE_THERAPY","ER_STATUS","PR_STATUS",'NPI',"RADIO_THERAPY")]

head(metabric)
metabric$status = factor(metabric$OS_STATUS ,c('0:LIVING','1:DECEASED'), labels = c(0,1))
...
devel <- data.frame(YY = with(metabric, Surv(round(OS_MONTHS), unclass(status))),
...
Zout <- readRDS("/Zout_os.rds")
fullfits <- readRDS("/fullfits_os.rds")
folds <- readRDS("/split_os.rds")
...
saveRDS(cv.auc, "/cv-aucs-stack_os.rds")
...
saveRDS(opt.fit, "/opt-coeffs_os_with_restriction.rds")
...
fullfits <- readRDS("/fullfits_os.rds")
cv.auc <- readRDS("/cv-aucs-stack_os.rds")
opt.fit <- readRDS("/opt-coeffs_os_with_restriction.rds")
...
print(xtable(sumtable[, 2:3], digits = 3))
...
index_model = order(-opt.fit$par)[c(1,8,16)]
temp = roc.opt
...
p1<- ggplot(temp, aes(x = fpf, y = tpf,color = method)) + geom_line() +
  scale_color_manual(
    values = c("#999999", "#E69F00",
               "#F0E442","#56B4E9"),
    labels = c( "CoxBoost" ,     "LASSO eventglm t4" ,'Optimized Stack',"SRF"         ))+
...
p2 <- ggplot(data.frame(pred.risk = Zopt.cut, pos = YYensvalid), aes(x = pred.risk, y = pos)) +
  ...
  theme_bw()
p1 + p2
#ggsave("C:/Users/pabgon/Documents/Project Megalearner/metabric/int-plot_metabric_os_restricted.pdf", width = 7.25, height = 3.75)
#ggsave("C:/Users/pabgon/Documents/Project Megalearner/metabric/int-plot_metabric_os_restricted1.pdf", width = 7.25, height = 3.75)
...
#ggsave("C:/Users/pabgon/Documents/Project Megalearner/metabric/km-fig_metabric_os_restricted.pdf", width = 5.25, height = 3.75)
